...
class NamedEntityRecognitionMEMM:
  def __init__(self):
        ...
        self.lr = 0.01525
        self.states = {'I', 'O', 'B'}
        self.lr_clf = LogisticRegression(penalty='l2', C=1.0, fit_intercept=True, 
        ...
        for e in examples:
            for (token, label) in e:
                ...
                for i in range(3):
                    ...
                    self.biases[i] = self.biases[i] - loss_i
  def parse_xy(self, examples):
        st_to_index = {'I':0, 'O':1, 'B':2}
        X = []
        y = []
        for s in examples:
            ...
            for e in s:
                (token, label) = e
                #print(label)
                X.append(self.featurize(token))
                y.append(st_to_index[label])                  
        return X, y     
      
  def train_model(self, examples):
       X, y = self.parse_xy(examples)
       self.lr_clf.fit(X, y)
       return self.lr_clf
  ...
  def gen_prob_2(self, data):
   ...
   # data = [i for i in data if i in self.vocab] #dealing with words in vocabulary. 
    ...
    parr = self.lr_clf.predict_proba([xi])[0]
...
if __name__ == "__main__":
    training = 'gene-trainF18-split90.txt'
    testing = 'gene-trainF18-split10.txt'
    ...
    training_examples = training_examples[0:100]
 ...
 #   mem2 = NamedEntityRecognitionMEMM()
 #   mem2.train_model(training_examples)
 #   mem2_answers = []
 #   for i in range(len(testing_examples)):
 #       e1 = testing_examples[i]
 #       tokens = [e[0] for e in e1]
 #       v, b = mem2.gen_prob_2(tokens)
 #       y = decode(tokens, v, b)
 #       #print("mem2 y = ", y)
 #       mem2_answers.append(y)
        
 #   mem_pred = []
 #   for e in mem2_answers:
 #       for t in e:
 #           mem_pred.append(t[1])
  #  print(real_labels[0:10], mem_pred[0:10])
  #  r2 = recall(real_labels, mem_pred)
    #p2 = precision(real_labels, mem_pred)
    #f_12 = f1(real_labels, mem_pred)
    
#    print("Model: MEMM with build in LogisticRegression {in progress}")
   # print("precision: ", p2)
#    print("recall: ", r2)
   # print("f1: ", f_12)
